...
LRESULT COpenGl::ClassProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_CREATE:
		...
		CheckMenuRadioItem( m_hViewMenu, VIEWMENU_PERSPECTIVE, VIEWMENU_DOWN, VIEWMENU_PERSPECTIVE, NULL );
		break;
...
*/
	...
	case WM_COMMAND:
		...
		case VIEWMENU_DOWN:
			...
			CheckMenuRadioItem( m_hViewMenu, VIEWMENU_PERSPECTIVE, VIEWMENU_DOWN, LOWORD(wParam), NULL );
			break;
		}
		::InvalidateRect( hWnd, NULL, FALSE );
		break;
	...
	case WM_KEYDOWN:
		...
		case 113: //F2
			GetWindowRect( m_hWnd, &rcWin );
			TrackPopupMenu( m_hViewMenu, TPM_LEFTALIGN, rcWin.left+1, rcWin.top+1, NULL, m_hWnd, NULL );
			break;
		}
		::InvalidateRect( hWnd, NULL, FALSE );
		break;
